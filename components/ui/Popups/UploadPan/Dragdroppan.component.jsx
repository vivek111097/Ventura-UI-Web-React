...
function Dragdropan(data) {
  const [errorMsg, SeterrorMsg] = useState('');
  const [isDocumentProceted,setisDocumentProceted]=useState(false);
      ...
      reader.onload = ()=> {
        ...
        files.text().then((x) => {
          if(x.includes("Encrypt")){
            ...
            setisDocumentProceted(true)
            console.log("file password protected")
            
          }
          else{
            setisDocumentProceted(false)
            getBase64(file)
          }
        });
      };
